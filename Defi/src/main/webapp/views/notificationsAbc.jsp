...
<c:choose>
	<c:when test="${nbNotificationsAbc == 0 }">
		<% out.print("no"); %>
	</c:when>
	<c:otherwise>
		<c:if test="${not empty mesChallengesRecus}">
			<br>
			<p style="margin-left: 10px; margin-right: 10px;">LES CHALLENGES A ACCEPTER POUR ABC</p>
		...
		<c:if test="${ not empty challengesEnAttentes}">
			<p style="margin-left: 10px; margin-right: 10px;">LES CHALLENGES A JOUES POUR ABC</p>
