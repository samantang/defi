...
<c:choose>
	<c:when test="${nbNotificationsDico == 0 }">
	<% out.print("no"); %>
	</c:when>
	<c:otherwise>
		<c:if test="${not empty mesChallengesRecus}">
			<br>
			<p style="margin-left: 10px; margin-right: 10px;">LES CHALLENGES A ACCEPTER POUR DICO</p>
		...
		<c:if test="${ not empty challengesEnAttentes}">
			<p style="margin-left: 10px; margin-right: 10px;">LES CHALLENGES A JOUES POUR DICO</p>
